...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        - seed: If not None, then pass this random seed to the dropout layers_dim. This
          will make the dropout layers_dim deteriminstic so we can gradient check the
          model.
        """
    ...
    def loss(self, X, y=None):
        ...
        for i in range(num_hidden_layers):
            ...
            if self.normalization is not None:
                if self.normalization == 'batchnorm':
                    hidden_norm, cache_bn = batchnorm_forward(hidden_in, self.params[str_gamma], self.params[str_beta], self.bn_params[i])
